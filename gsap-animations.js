...
const PLACEHOLDER_IMG = "data:image/svg+xml,%3Csvg viewBox='0 0 2 3' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='2' height='3' fill='%230a0a0a'/%3E%3C/svg%3E";

const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
const masterContext = gsap.matchMedia();
...
// Step 2: split text while preloader overlay is still visible
splitLines(document);
...
function initTheme() {
  ...
  const initThemeVal = document.documentElement.getAttribute('data-theme');
  const systemLight = window.matchMedia('(prefers-color-scheme: light)').matches;
  if (toggle) {
    toggle.textContent = (initThemeVal === 'light' || (!initThemeVal && systemLight)) ? 'Dark' : 'Light';
  }
}
...
async function loadArchiveData() {
  try {
    ...
    archiveData = { locations: ['Heritage Estate'], films: [] };
    for (let i = 1; i <= 26; i++) {
      archiveData.films.push({ id: i, name: `Archive Frame ${i}`, image: `images/C${i}.jpg` });
    }
  }
}

/* ── Step 1: Archive with lazy images, no preloader blocking ── */
function buildFilmArchive() {
  ...
  const lazyImages = track.querySelectorAll('.lazy-film-img');
  ...
  setupDesktopFilmsScroll();
  initFilmFrameReveal();
  ScrollTrigger.refresh();
}

function setupDesktopFilmsScroll() {
...
/* ── Step 1: Preloader tracks hero only ── */
const preloader = document.getElementById('preloader');
const preloaderFill = document.getElementById('preloaderFill');
const criticalImages = [document.querySelector('.opening-media img')].filter(Boolean);
let loaded = 0;
...
function imageLoaded() {
  ...
  gsap.to(preloaderFill, { width: `${pct}%`, duration: 0.2, ease: 'power1.out' });
  if (loaded >= criticalImages.length) tryFinishPreload();
}
...
/* ── Step 3: Cursor scoped to desktop with teardown ── */
function initCursor() {
  ...
  const cursorElements = document.querySelectorAll('[data-cursor], .film-frame, .reel-cta, .form-submit, a, button');
  const handlers = [];
  ...
  cursorElements.forEach((el) => {
    ...
    el.addEventListener('mouseenter', enterTarget);
    el.addEventListener('mouseleave', leaveTarget);
  ...
  return () => {
    ...
    handlers.forEach(({ el, enterTarget, leaveTarget }) => {
      el.removeEventListener('mouseenter', enterTarget);
      el.removeEventListener('mouseleave', leaveTarget);
    });
...
function initForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!valid) {
      gsap.to(submitBtn, { x: 10, repeat: 3, yoyo: true, duration: 0.05, clearProps: 'x' });
      return;
    }
    ...
    try {
      ...
      submitBtn.textContent = 'Audience Requested';
      submitBtn.style.borderColor = 'var(--gold)';
      submitBtn.style.background = 'rgba(200, 168, 107, 0.1)';
...
function initSite() {
  ...
  masterContext.add('(min-width: 901px)', () => {
    cursorCleanup = initCursor();
    if (archiveBuilt) setupDesktopFilmsScroll();
    return () => {
      if (cursorCleanup) { cursorCleanup(); cursorCleanup = null; }
      filmsScrollReady = false;
      ScrollTrigger.getAll().forEach((st) => {
        if (st.vars?.trigger === '#films' || st.trigger === document.getElementById('films')) {
          st.kill();
        }
      });
    };
  ...
  masterContext.add('(max-width: 900px)', () => {
    const filmsSection = document.getElementById('films');
    filmsSection?.classList.add('is-mobile');
    gsap.set('#filmsTrack', { x: 0 });
    gsap.set('#filmsProgress', { width: 0 });
  ...
  gsap.to('.opening-media', {
    ...
    ease: 'none',
    force3D: true,
    scrollTrigger: { trigger: '#opening', start: 'top top', end: 'bottom top', scrub: 0.35 },
  });
  ...
  gsap.to('.opening-copy', {
    ...
    ease: 'none',
    force3D: true,
    scrollTrigger: { trigger: '#opening', start: 'top top', end: '60% top', scrub: true },
  });
  ...
  gsap.from('#portraitImg img', {
    ...
    ease: 'power3.inOut',
    force3D: true,
    scrollTrigger: { trigger: '#portrait', start: 'top 70%', end: 'top 20%', scrub: 0.35 },
  });
  ...
  gsap.from('.stakes-img.primary img', {
    ...
    ease: 'none',
    force3D: true,
    scrollTrigger: { trigger: '#stakes', start: 'top bottom', end: 'bottom top', scrub: 0.4 },
  });
  ...
  gsap.to('#reelBg', {
    ...
    ease: 'none',
    force3D: true,
    scrollTrigger: { trigger: '#reel', start: 'top bottom', end: 'bottom top', scrub: 0.35 },
  });

  gsap.to('#chromatic', {
    opacity: 0.6,
    ease: 'none',
    scrollTrigger: { trigger: '#reel', start: 'top center', end: 'bottom center', scrub: true },
  });
